...
def _registry(tmp_path, owner_row=ROW, code=True):
    """``code`` puts a typed override code on the owner row, and it is on by
    default because the merge with people-signin made it the difference
    between an enforcing gate and a shadow one: ``gate._mode_unsafe``
    downgrades enforce to SHADOW when no owner has a way back in. Shadow
    admits everybody with ``who=""``, which is a green suite that measures
    nothing about lockout -- see tests/test_voice_multispeaker_wiring::_gate
    for the same note and the same one-line fix.
    """
    ...
    if code:
        reg.set_secret(owner_row, "code_hash", pp.hash_secret("xxx000code"))
    reg.save()
