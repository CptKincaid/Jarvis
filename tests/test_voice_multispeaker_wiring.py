...
def _gate(tmp_path, mode="enforce", code=True):
    """A gate over one owner and one known person.

    ``code`` SETS AN OVERRIDE CODE ON THE OWNER ROW, and it defaults on
    because without one this helper does not build an enforcing gate at
    all. That is not this branch's rule: the people-signin lane added
    ``gate._mode_unsafe``'s lockout guard, which downgrades enforce to
    SHADOW whenever no owner carries a typed way back in, on the ground
    that a wrong verdict would otherwise have none. Every fixture on that
    lane sets one for the same reason (tests/test_owner_gate.py::_registry,
    tests/test_owner_gate_wiring.py::_stand_in) and these did not, because
    they were written before the guard existed.

    IT WAS NOT ONLY THE RED TESTS. In shadow every judge() returns
    ``admit=True, who=""`` with ``would_refuse`` set, so the eleven
    assertions that read ``d.who``/``d.admit`` went red -- but the counting
    tests above ("0 of 150 of her clips were admitted as him") would have
    stayed GREEN and stopped measuring anything, because nobody is ever
    admitted as anybody in shadow. The guard this fixture restores is the
    one the whole file is about.
    """
