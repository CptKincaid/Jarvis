...
from jarvis.speaker import (MIN_AUDIO_SECONDS, MIN_SPEECH_SECONDS, SAMPLE_RATE,
                            SpeakerVerifier, speech_bounds, trim_silence)
...
def test_a_short_wake_word_can_be_scored_when_the_caller_asks(monkeypatch):
    ...
    assert v.score(clip, min_seconds=MIN_SPEECH_SECONDS) is not None
...
def test_every_other_caller_keeps_the_full_second(monkeypatch):
    ...
    monkeypatch.setattr(v, "_extract_embedding",
                        lambda a, min_seconds=1.0: seen.append(min_seconds) or real(a, min_seconds))
